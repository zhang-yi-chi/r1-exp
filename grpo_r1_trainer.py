...
from r1_experience_maker import NewExperienceMaker
...
class GRPOTrainer(ABC):
    ...
    def __init__(
        ...
        **generate_kwargs,
        ...
        self.experience_maker = NewExperienceMaker(
            actor,
            None,
            reward_model,
            initial_model,
    ...
    def fit(
        ...
        for episode in range(start_episode, args.num_episodes):
            ...
            for rand_prompts, rand_answers in self.prompts_dataloader:
                for i, experience in enumerate(
                    self.experience_maker.make_experience_list(rand_prompts, rand_answers, **self.generate_kwargs)
                ):
                    if i == 0:
    ...
    def evaluate(self, eval_dataloader, steps=0):
        ...
        with torch.no_grad():
            ...
            status = {}
            for rand_prompts, rand_answers in step_bar:
                for i in range(0, len(rand_prompts), self.args.micro_rollout_batch_size):
                    prompts = rand_prompts[i : i + self.args.micro_rollout_batch_size]
                    inputs = self.experience_maker.tokenize_fn(prompts, self.prompt_max_len, device="cuda")
                    sequences, attention_mask, action_mask = self.actor.generate(**inputs, **self.generate_kwargs)
                    prompts_length = inputs["input_ids"].size(1)
                    completion_ids = sequences[:, prompts_length:]
                    answers = rand_answers[i : i + self.args.micro_rollout_batch_size]
                    ...
                    r = torch.zeros((len(completions),), dtype=torch.float32, device=sequences.device)
                    for k, v in reward_dict.items():
                        v = torch.tensor(v, dtype=torch.float32, device=sequences.device)
                        if k not in status:
                            status[k] = []
                        status[k].append(v.mean().item())
            for k, v in status.items():
                if len(v) > 0:
                    status[k] = sum(v) / len(v)
                else:
                    status[k] = 0
            logs = self.strategy.all_reduce(status)
