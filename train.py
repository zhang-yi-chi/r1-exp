...
def train(args):
    ...
    # 指定数据集，对应 R1Dataset 里的不同处理方法
    prompts_dataset = R1Dataset(prompts_data, tokenizer, strategy, input_template=args.input_template)
    eval_dataset = R1Dataset(eval_data, tokenizer, strategy, input_template=args.input_template)
    ...
    eval_dataloader = strategy.setup_dataloader(
        eval_dataset, args.rollout_batch_size // strategy.world_size, True, False
    )
    if args.pretrain_data:
    ...
    reward_model = REWARD_FUNCTIONS[args.reward_fn]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--reward_fn", type=str, default=None, help="reward function name defined in reward.py")
    parser.add_argument(
        "--prompt_data_probs",
        type=str,
        default="1.0",
    ...
    assert args.advantage_estimator.lower() == "grpo", "GRPO"
